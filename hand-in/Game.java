...
public class Game {
    ...
    * /!\ you don't need to change this function! */
    ...
    int createPlayer(Connection conn, Player person) throws SQLException {
        String countryQuery = "SELECT country FROM areas WHERE name = ?";
        PreparedStatement countrySt = conn.prepareStatement(countryQuery);
        ...
        try {
            ...
            countrySt.setString(1, person.getStartingArea());
            conn.commit();
            ResultSet rs = countrySt.executeQuery();
            String country;
            if (rs.next()) {
            ...
            st.setString(5, person.getStartingArea());
            st.setInt(6, 1000);
            st.executeUpdate();
            conn.commit();
            st.close();
